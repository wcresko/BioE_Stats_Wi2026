...
![](images/pca_variance_maximization.svg){fig-align="center" width="95%"}
...
![](images/sne_to_tsne.svg){fig-align="center" width="95%"}
...
UMAP (McInnes, Healy & Melville, 2018) achieves similar goals to t-SNE but approaches the problem from **algebraic topology** rather than information theory. The core assumption is that the data lies on a **manifold** — a curved surface embedded in high-dimensional space — and that the manifold is locally approximately uniform.
...
![](images/umap_algorithm.svg){fig-align="center" width="95%"}
...
![](images/tsne_vs_umap_comparison.svg){fig-align="center" width="95%"}


## Key Differences Between t-SNE and UMAP

Despite the similarities, three differences matter in practice:

**1. Speed and scalability:** UMAP uses stochastic gradient descent with negative sampling, giving it approximately O(n) time per epoch versus t-SNE's O(n log n) with Barnes-Hut approximation. For datasets above \~50,000 points, UMAP is dramatically faster.

**2. Global structure preservation:** t-SNE's KL divergence is asymmetric — it heavily penalizes placing true neighbors far apart (good), but barely penalizes placing distant points close together. This means the *relative positions* of clusters in a t-SNE plot are essentially meaningless. UMAP's cross-entropy loss is symmetric, meaning it also pushes distant points apart, better preserving the global topology. Clusters that are biologically related tend to appear closer together in UMAP than in t-SNE.

**3. Theoretical foundation:** UMAP is grounded in topological data analysis, which provides theoretical justification for why the algorithm works and for the meaning of its parameters. t-SNE is more heuristic — the t-distribution was chosen because it works well, not because it follows from a mathematical theory.

::: callout-note
For most bioengineering applications, UMAP is now the default choice for non-linear visualization due to its speed and better global structure preservation. t-SNE remains valuable for small datasets where you want to carefully explore local structure at multiple perplexity settings.
:::
...
![](images/clustering_methods_overview.svg){fig-align="center" width="95%"}

## Choosing a Clustering Method

| Question | K-Means | Hierarchical | KNN |
|:-----------------|:-----------------|:-----------------|:-----------------|
| **Supervised?** | No | No | Yes (needs labels) |
| **Need to specify k?** | Yes, in advance | No, cut tree later | Yes (k neighbors) |
| **Cluster shape** | Spherical | Any shape | Flexible boundaries |
| **Scalability** | Excellent (large n) | Poor (O(n²)) | Moderate |
| **Deterministic?** | No (random init) | Yes | Yes |
| **Output** | Cluster assignments | Dendrogram | Predicted class |

::: callout-note
**K-Means** and **Hierarchical clustering** are **unsupervised** — they discover groups without using any labels. **KNN** is **supervised** — it requires labeled training data and classifies new observations based on their nearest labeled neighbors.
:::
...
# K-means with k=4, visualized in PCA space
set.seed(2026)
km_fit <- kmeans(inflam_scaled, centers = 4, nstart = 25)

plot(pca_scores$PC1, pca_scores$PC2, 
     col = c("#3498db", "#e74c3c", "#2ecc71", "#9b59b6")[km_fit$cluster],
     pch = 19, cex = 1.2,
     xlab = paste0("PC1 (", var_pct[1], "%)"),
     ylab = paste0("PC2 (", var_pct[2], "%)"),
...
| **Nested structure?** | No | Yes (dendrogram shows hierarchy) |

::: callout-tip
Use the **elbow method** or **silhouette scores** to choose k for k-means. Consider running both k-means and hierarchical clustering and comparing results — agreement between methods increases confidence in the groupings.
:::
...
![](images/knn_concept.svg){fig-align="center" width="95%"}
...
-   **Small K** (e.g., K = 1) makes very local decisions — high variance, may overfit to noise
-   **Large K** makes smoother decisions — may underfit and miss real boundaries
...
-   For this well-separated dataset, many values of K work well — in datasets with more overlap between groups, the choice of K matters more
:::

## When to Use KNN in Bioengineering

**KNN is a good choice when:**

-   **You have labeled training data** and want to classify new observations into known categories
-   **Decision boundaries are non-linear** — KNN naturally handles complex class boundaries without specifying a model form
-   **You need a quick baseline classifier** before investing in more complex methods like random forests or SVMs
-   **Sample sizes are moderate** (100s to low 1000s) — KNN scales poorly to very large datasets

**Bioengineering examples:**

-   **Cell type classification** — given expression profiles of labeled cell types, classify new unlabeled cells
-   **Implant outcome prediction** — given patient features from previous implant recipients with known outcomes (success/failure), predict outcome for a new patient
-   **Tissue identification** — given spectroscopic signatures from known tissue types, classify biopsy samples
-   **Drug response classification** — given gene expression profiles of known responders/non-responders, classify a new patient's likely response

::: callout-tip
KNN works best when the number of features is moderate relative to the sample size. With very high-dimensional data (e.g., 20,000 genes), reduce dimensions first with PCA and apply KNN on the top PCs.
:::
...
| Continuous | Categorical (repeated) | Repeated measures / mixed model | 7–8 |
...
|:---|:---|:---|:---|
| Classify | Labeled, few features | Logistic regression, LDA | 8 |
| Classify | Labeled, complex boundaries | KNN, random forest | 9–10 |
...
| Find groups | No labels | K-means, hierarchical clustering | 10 |
...
## Congratulations! {background-color="#27ae60"}

You've made it through **Statistics for Bioengineering**! Over 10 weeks, you've built a comprehensive toolkit:

-   **Weeks 1–3:** Foundations — experimental design, probability, distributions, sampling, and descriptive statistics
-   **Week 4:** Hypothesis testing — t-tests, confidence intervals, power analysis, and multiple testing correction
-   **Week 5:** Non-parametric methods — rank-based tests, bootstrapping, and permutation tests
-   **Week 6:** Simple linear regression and correlation — model fitting, diagnostics, and ANOVA
-   **Week 7:** Multiple regression, ANCOVA, factorial ANOVA, nested designs, and repeated measures
-   **Week 8:** Linear mixed models (lme4), generalized linear models (logistic and Poisson), and survival analysis
-   **Week 9:** Nonlinear smoothing (GAMs, splines), statistical learning (CART, random forests), and introduction to Bayesian inference (brms)
-   **Week 10:** Multivariate methods — PCA, PCoA, NMDS, t-SNE, UMAP, hierarchical clustering, K-means, and KNN
