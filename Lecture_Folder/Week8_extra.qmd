...
**Datasets for This Week:** `glp1_simple.csv`, `glp1_diet.csv`, `scaffold_degradation_data.csv` — all in the `data/` folder. Additional datasets from packages: `lung` (survival package)

# Finish Linear Mixed Models {background-color="#d4edda"}

## Where We Left Off

Last week we introduced the concept of linear mixed models — models that partition variability into **fixed effects** (treatments you manipulate) and **random effects** (nuisance grouping variables like subjects, batches, or labs).

Today we'll build intuition for *why* mixed models work by starting with a simple example and progressively adding complexity. We'll see how moving from a fixed-effects-only model to a random-effects model changes our residuals, our inference, and our understanding of the data.

::: callout-tip
### Key Insight

The core idea: subjects (mice, batches, patients) differ from each other in systematic ways. If we ignore this, we either waste degrees of freedom (treating each subject as a fixed effect) or inflate Type I error (ignoring the grouping entirely). Mixed models solve both problems.
:::

## Building Intuition: The GLP-1 Weight Loss Study

We'll work through two versions of a GLP-1 receptor agonist weight loss experiment:

1.  **Simple version:** 6 mice, all on GLP-1, weighed weekly for 10 weeks — introduces random intercepts
2.  **Extended version:** 12 mice on GLP-1, half on high-protein diet and half on low-protein diet — introduces random slopes and treatment interactions

For each, we'll fit three models of increasing sophistication and compare how well they capture the data structure.

# Example 1: Six Mice on GLP-1 {background-color="#d4edda"}

## The Experiment

Six mice are placed on a GLP-1 receptor agonist (semaglutide). Each mouse is weighed weekly for 10 weeks. All mice lose weight over time, but they start at different baseline weights and lose weight at slightly different rates.

**The statistical question:** Is there a significant decline in weight over the 10-week treatment period?
...
-   6 mice (M1–M6), each weighed weekly for 10 weeks (weeks 0–10), yielding 66 observations
-   Each mouse starts at a different baseline weight (\~30–38 g)
-   All mice lose weight at approximately 0.45 g/week, with small individual variation
-   Residual measurement noise is roughly 0.6 g
...
ggplot(glp1_simple, aes(x = week, y = weight, color = mouse_id, group = mouse_id)) +
  ...
  labs(
    title = "GLP-1 Weight Loss Study: 6 Mice Over 10 Weeks",
...
ggplot(glp1_simple, aes(x = week, y = weight, 
  ...
  geom_point(size = 2, alpha = 0.7) +
  scale_color_brewer(palette = "Set2") +
  labs(title = "GLP-1 Weight Loss Study: 6 Mice Over 10 Weeks",
       x = "Week", y = "Weight (g)", color = "Mouse")
```
...
-   A standard `lm()` that ignores mouse identity would treat all 66 points as independent, which they clearly aren't
:::

## Model 1a: Ignoring the Grouping (Naive lm)
...
-   This model treats all 66 observations as independent — it ignores the mouse grouping entirely
...
-   The standard error for the slope is inflated, making our test of the time effect less powerful
:::

## Model 1b: Mouse as a Fixed Effect (Parallel Slopes)
...
## Model 1c: Random Intercept Model (lmer)
...
-   The **naive model** has the largest SE because between-mouse variance inflates the residual — it also has the most residual df but that's misleading since observations aren't independent
-   The **fixed mouse model** has the smallest SE but burns df on nuisance parameters and can't generalize
-   The **lmer model** provides a good compromise: an appropriately sized SE, honest accounting for the non-independence, and results that generalize to the population of mice
:::
...
Now suppose we have 12 mice, all on GLP-1, but randomized into two diet groups: 6 on a **high-protein diet** and 6 on a **low-protein diet**. We measure weight weekly for 10 weeks.
...
-   12 mice total: 6 per diet group, each weighed weekly for 10 weeks (132 observations)
...
ggplot(glp1_diet, aes(x = week, y = weight, color = mouse_id, 
  ...
  facet_wrap(~diet) +
  labs(
    title = "GLP-1 + Diet Study: 12 Mice Over 10 Weeks",
...
-   But this model treats all 132 observations as independent — it doesn't know that the 11 measurements on mouse M1 are correlated
...
-   The **naive model** has the smallest SE — but this is misleadingly small because it treats all 132 observations as independent
-   The **lmer model** has a larger (and more honest) SE that accounts for within-mouse correlation
-   The lmer p-value is the one you should report — it properly accounts for the fact that you have 12 independent experimental units (mice), not 132
...
1.  **Naive `lm()`**: Pretend all observations are independent. Fast, simple, wrong. Inflated Type I error.
...
When selecting a model, the nature of your response variable guides the choice:

![](images/model_decision_flowchart.svg){fig-align="center" width="90%"}

The remainder of this lecture introduces the methods on the right side of this flowchart: GLMs (logistic, Poisson), regularization, splines, and survival analysis.
